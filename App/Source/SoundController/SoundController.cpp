...
bool CSoundController::Init(void)
{
	...
	cSoundEngine->setSoundVolume(0.01f);
	for (int i = 0; i < CSoundInfo::NUM_SOUNDGROUP; i++)
	{
		std::vector<int> Temp;
		SoundSystem.push_back(Temp);
	}
	for (int i = 0; i < 5; ++i)
	{
		if (i == 0)
...
bool CSoundController::Update(const double dElapsedTime)
{
	for (int i = 0; i < SoundSystem.size(); ++i)
	{
		for (int j = 0; j < SoundSystem.at(i).size(); ++j)
		{
			// Get the ISoundSource
			ISoundSource* pISoundSource = GetSound(SoundSystem.at(i).at(j))->GetSound();
			if (pISoundSource == nullptr)
			{
				return false;
			}
			pISoundSource->setDefaultVolume(VolumeValue[i+1]);
		}
	}
}
...
bool CSoundController::LoadSound(	string filename,
									...
									CSoundInfo::SOUNDGROUP eSoundGroup,
									...
									vec3df vec3dfSoundPos)
{
	...
	if (eSoundType == CSoundInfo::SOUNDTYPE::_2D)
		cSoundInfo->Init(ID, pSoundSource, bIsLooped, eSoundGroup);
	else
		cSoundInfo->Init(ID, pSoundSource, bIsLooped, eSoundGroup,eSoundType, vec3dfSoundPos);
...
void CSoundController::LoadSoundSystem(void)
{
	for (std::map<int, CSoundInfo*>::const_iterator it = soundMap.begin(); it != soundMap.end(); ++it)
	{
		bool Existing = false;
		if ((*it).second != NULL)
		{
			for (int i = 0; i < SoundSystem.size()-1; i++)
			{
				for (int j = 0; j < SoundSystem.at(i).size(); j++)
				{
					if (it->first == SoundSystem.at(i).at(j))
						Existing = true;
				}
			}
			if (Existing == false)
			{
				SoundSystem.at(it->second->GetSoundGroup()).push_back(it->first);
			}
		}
	}
}
...
void CSoundController::PlaySoundByID(const int ID)
{
	...
	else if (cSoundEngine->isCurrentlyPlaying(pSoundInfo->GetSound()) && pSoundInfo->GetLoopStatus() == true)
	{
 ...
 */
bool CSoundController::MasterVolumeIncrease( float volume)
{
	// Get the current volume
	float fCurrentVolume = cSoundEngine->getSoundVolume() + volume / 100;
 ...
 */
bool CSoundController::MasterVolumeDecrease(float volume)
{
	// Get the current volume
	float fCurrentVolume = cSoundEngine->getSoundVolume() - volume / 100;
...
{
	float fCurrentVolume = VolumeValue[0];
...
bool CSoundController::SoundGroupVolumeIncrease(CSoundInfo::SOUNDGROUP eSoundGroup, float volume)
{
	// Get the current volume
	float fCurrentVolume = VolumeValue[eSoundGroup + 1] + volume / 100;
	...
	// Decrease the volume
	VolumeValue[eSoundGroup + 1] = fCurrentVolume;
	return true;
}

bool CSoundController::SoundGroupVolumeDecrease(CSoundInfo::SOUNDGROUP eSoundGroup, float volume)
{
	// Get the current volume
	float fCurrentVolume = VolumeValue[eSoundGroup+1] - volume / 100;
...
float CSoundController::GetSoundGroupVolume(CSoundInfo::SOUNDGROUP eSoundGroup)
{
	float fCurrentVolume = VolumeValue[eSoundGroup+1];
	return fCurrentVolume;
}
