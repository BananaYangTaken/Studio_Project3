 ...
 */
CEnemy2D_Mutant::CEnemy2D_Mutant(void)
{
	bIsActive = false;
	cMap2D = NULL;
	cSettings = NULL;
...
/**
  ...
  */
bool  CEnemy2D_Mutant::checkforLOS()
{
...
bool CEnemy2D_Mutant::Init(int x, int y)
{
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Image/MutantZombie.png", true);
	if (iTextureID == 0)
	{
		cout << "Unable to load Image/MutantZombie.png" << endl;
		return false;
	}
	...
	animatedSprites->AddAnimation("charge", 68, 70);
	deathTimer = 0;
...
void CEnemy2D_Mutant::UpdateToLastLOS()
{
...
void CEnemy2D_Mutant::droploot()
{
	...
	{
		float recvey = vec2Index.x, recvex = vec2Index.y;
...
void CEnemy2D_Mutant::spawnloot(float vecX, float vecY)
{
	...
	case ATTACK:
		...
		if (cPhysics2D.CalculateDistance(vec2Index, dynamic_cast<CPlayer2D_V2*>(Player)->vec2Index) < 1.5f)
		{
			...
			else if (Attack && AttackAnim <= 0)
			{
				...
				{
					dynamic_cast<CPlayer2D_V2*>(Player)->SetHealth(dynamic_cast<CPlayer2D_V2*>(Player)->GetHealth() - 15);
