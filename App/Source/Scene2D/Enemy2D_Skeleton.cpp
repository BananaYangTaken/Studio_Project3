 ...
 */
CEnemy2D_Skeleton::CEnemy2D_Skeleton(void)
{
	bIsActive = false;
	cMap2D = NULL;
	cSettings = NULL;
...
/**
  ...
  */
bool CEnemy2D_Skeleton::Init(int x, int y, std::vector<CProjectile2D*>* cProjectileList)
{
...
CPlayer2D_V2* CEnemy2D_Skeleton::DecideTarget(void)
{
	return Player2;
}


void CEnemy2D_Skeleton::Update(const double dElapsedTime)
{
	if (!bIsActive)
		return;
	...
	case ATTACK:
		...
		if (!Stunned)
		{
			if (cPhysics2D.CalculateDistance(vec2Index, DecideTarget()->vec2Index) < 7.0f)
			{
				...
				// Attack
				if (cPhysics2D.CalculateDistance(vec2Index, DecideTarget()->vec2Index, 'x') < 8.0f && Attack == false)
				{
					Attack = true;
					AttackAnim = 1;
					UpdateDirection(); 
				...
				if (Attack && AttackAnim <= 0)
				{
					...
					cProjectile->Init(vec2Index, vec2NumMicroSteps, Direction, 9, CProjectile2D::HOSTILE, CProjectile2D::ARROW);
					Attack = false;
