 ...
 */
CEnemy2D_Police::CEnemy2D_Police(void)
{
	bIsActive = false;
	cMap2D = NULL;
	cSettings = NULL;
...
/**
  ...
  */
bool  CEnemy2D_Police::checkforLOS()
{
...
bool CEnemy2D_Police::Init(int x, int y)
{
	...
	if (iTextureID == 0)
	{
		cout << "Unable to load Image/PoliceZombie.png" << endl;
		return false;
	}
...
void CEnemy2D_Police::UpdateToLastLOS()
{
...
void CEnemy2D_Police::spawnloot(float vecX, float vecY)
{
...
void CEnemy2D_Police::Update(const double dElapsedTime)
{
	UpdateDirection();
	checkforLOS();
