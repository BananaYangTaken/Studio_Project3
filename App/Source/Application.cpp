...
bool Application::Init(void)
{
	...
	//Initialise the CSoundController Instance
	if (CSoundController::GetInstance()->Init() == false)
	{
		std::cout << "Failed to load SoundController" << std::endl;
		return false;
	}
	/* Old Scene Code
	...
	{
		std::cout << "Failed to load Scene2D" << std::endl;
		return false;
	}
	*/
...
void Application::Run(void)
{
	...
	while (!glfwWindowShouldClose(cSettings->pWindow)
		//&& (!CKeyboardController::GetInstance()->IsKeyReleased(GLFW_KEY_ESCAPE)) //Bitch Code
		)
	{
		...
		*/
		...
		// Comment out this 2 line to remove Frame Limiter
